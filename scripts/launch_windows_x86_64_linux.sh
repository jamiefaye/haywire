...
QEMU_ARGS=(
    ...
    # Graphics - VNC on all interfaces for Windows access
    -vga std
    -vnc 0.0.0.0:1
...
# Use the same QEMU binary as the Ubuntu VM
QEMU_BIN="/home/jamie/qemu-build/build-x86/qemu-system-x86_64"
if [ ! -f "$QEMU_BIN" ]; then
    QEMU_BIN="qemu-system-x86_64"
    echo "Using system QEMU (custom build not found)"
else
    echo "Using custom QEMU build: $QEMU_BIN"
fi
