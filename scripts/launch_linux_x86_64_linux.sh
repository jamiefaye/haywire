...
# Launch x86_64 Ubuntu on Intel Linux with KVM acceleration
# This uses KVM for NATIVE performance (not emulation!)

UBUNTU_ISO="/path/to/ubuntu-24.04-desktop-amd64.iso"
...
echo "This should be FAST on Intel Linux - using native CPU!"
echo ""
...
if [ ! -f "$UBUNTU_ISO" ]; then
    ...
    echo ""
    echo "Update UBUNTU_ISO path in this script"
    exit 1
fi
...
if [ ! -e /dev/kvm ]; then
    ...
    echo "Make sure:"
    echo "  1. KVM kernel modules are loaded (modprobe kvm kvm_intel)"
    echo "  2. Your user is in the 'kvm' group (sudo usermod -aG kvm $USER)"
    echo "  3. Virtualization is enabled in BIOS"
    exit 1
fi

echo "Starting QEMU with KVM acceleration (native Intel)..."
